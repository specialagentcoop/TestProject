...
connection = psycopg2.connect(database='cvmarket_data', host='localhost', user='postgres', password='Ugne13')
cursor = connection.cursor() # prisijungimas prie duomenų bazės
create_table_query = """
    CREATE TABLE IF NOT EXISTS it_jobs_data(
...
for url in url_list:
    ...
    for job in jobs:
        ...
        insert_query = ('INSERT INTO it_jobs_data(title, company, min_salary, max_salary, salary_type, location, job_posted) VALUES(%s, %s, %s, %s, %s, %s, %s)')
        cursor.execute(insert_query, (title, company, salary_min, salary_max, salary_type, location, job_posted))
    connection.commit()
